...
PYTHON_DEPLOY_TEMPLATE = """FROM python:3.11-slim
WORKDIR /app
COPY . .
RUN pip install --no-cache-dir -r requirements.txt 2>/dev/null || true
# Install common web servers just in case
RUN pip install uvicorn flask fastapi 2>/dev/null || true
EXPOSE 8000
CMD {cmd}
"""

NODE_DEPLOY_TEMPLATE = """FROM node:20-slim
WORKDIR /app
COPY package*.json ./
...
class OrchestratorAgent:
        ...
        try:
            ...
            while state.current_iteration < state.retry_limit:
                ...
                if monitor_result["should_stop"]:
                    ...
                    # Push partial fixes to remote (so they’re available on GitHub)
                    if state.cumulative_fixes > 0:
                        self._git_push(state)
            ...
            # Clean up Docker images at the very end (cached across iterations)
            try:
                subprocess.run(["docker", "rmi", "-f", "cicd-healing-base-image"], capture_output=True, timeout=30)
            except Exception:
                pass
    ...
    def _git_push(self, state: SharedState):
        ...
        try:
            ...
            # Not fatal - user might not have write access
            state.git_push_status = f"Failed (Permission/Auth): {e.stderr.strip()[:50]}..."
            logger.warning(f"[OrchestratorAgent] ⚠️ Git push failed (proceeding to deploy): {e.stderr}")
    ...
    def _deploy_container(self, state: SharedState):
        ...
        img_tag = f"cicd-deploy-{state.branch_name.lower().replace('_', '-')}"
        
        try:
            # 1. Ensure Dockerfile exists
            if not os.path.exists(os.path.join(state.repo_path, "Dockerfile")):
                logger.info("[OrchestratorAgent] No Dockerfile found. Generating one for deployment...")
                self._generate_deploy_dockerfile(state)
            ...
            # 3. Cleanup old container if exists
            container_name = f"deploy-{port}"
            subprocess.run(["docker", "rm", "-f", container_name], capture_output=True)
            
            # 4. Run
            inner_port = 8000 if state.language == "python" else 3000
            ...
            state.deployment_url = f"Failed: {str(e)[:100]}"

    def _generate_deploy_dockerfile(self, state: SharedState):
        """Generates a minimalistic Dockerfile for deployment based on language."""
        path = os.path.join(state.repo_path, "Dockerfile")
        ...
        if state.language == "python":
            ...
            if os.path.exists(os.path.join(state.repo_path, "main.py")):
                cmd = '["python", "main.py"]'
            elif os.path.exists(os.path.join(state.repo_path, "manage.py")):
                cmd = '["python", "manage.py", "runserver", "0.0.0.0:8000"]'
            
            content = PYTHON_DEPLOY_TEMPLATE.format(cmd=cmd)
            
        elif state.language == "node":
            cmd = '["npm", "start"]'
            content = NODE_DEPLOY_TEMPLATE.format(cmd=cmd)
        
        else:
             # Fallback generic
             content = "FROM alpine\nCMD echo 'Unsupported language for auto-deploy'"
